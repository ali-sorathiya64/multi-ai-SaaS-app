...
const DocumentSummary = () => {
  ...
  const onSubmitHandler = async (e) => {
    ...
    try{
      ...
      const {data} = await axios.post('/api/ai/summary-docs',formData ,{headers: {Authorization: `Bearer ${await getToken()}`}});
    ...
    <div className='h-full overflow-y-scroll p-4 sm:p-6 flex flex-col lg:flex-row items-start gap-6 text-slate-700'>
      ...
      >
        <div className='flex items-center gap-2 sm:gap-3'>
          <Sparkles className='w-5 sm:w-6 flex-shrink-0 text-[#00AD83]' />
        ...
        <p className='mt-5 text-sm font-medium'>Upload Document (PDF)</p>
        <input
        ...
        <p className='text-xs text-gray-500 font-light mt-1'>
          Supports PDF files — e.g., syllabus, notes, reports, resumes, etc.
        </p>

        <button disabled={loading}
          type='submit'
          className='w-full flex justify-center items-center gap-2 bg-gradient-to-r 
          from-[#00DA83] to-[#009BB3] text-white px-4 py-2 sm:py-3 mt-6 text-sm sm:text-base rounded-lg cursor-pointer'
        > {loading ? <span className='w-4 h-4 my-1 rounded-full border-2 border-t-transparent animate-spin'>

        </span> :  <FileText className='w-4 sm:w-5' />}
      ...
      {/* Right Section */}
      <div className='w-full max-w-lg p-4 sm:p-6 bg-white rounded-lg flex flex-col border border-gray-200 min-h-96 max-h-[600px]'>
        <div className='flex items-center gap-2 sm:gap-3'>
  ...
  {!content ? ( <div className='flex-1 flex justify-center items-center'>
          <div className='text-sm flex flex-col items-center gap-5 text-gray-600'>
            ...
            <p>
              Upload a document and click <strong>“Generate Summary”</strong> to get started.
            </p>
          </div>
